...
rot_theta_vec = [-0.07] #np.round(np.linspace(-0.09,-0.05,17),5)
rot_phi_vec = [0.02] #np.round(np.linspace(-0.015,-0.025,11),5)
cor_vec = np.round(np.linspace(0.015,0.025,11),5)
Z_pos_vec =[5]
y_pos_vec = [0]
Z2_vec = [20] #[24-11.9]
rot_dir_vec = [-1]
                ...
                for rot_phi in rot_phi_vec:
                    for CoR in cor_vec:
                        for rot_dir in rot_dir_vec:
                            ...
                            Acq_data2.geometry.config.system.detector.position[2] = y_pos
                            ...
                            cent_slice.save(os.path.join(outFolder, f'{ii}_CoR_{CoR}_theta_{rot_theta}_phi_{rot_phi}_Z_{Z_pos}_Z2_{Z2_pos}_Y_{y_pos}_dir_{rot_dir}.tif'), format="TIFF", tile=True)
