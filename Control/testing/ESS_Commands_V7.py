...
def SendAsciiCmd(ser, cmd):
    cmd = cmd + '\r'  # Append carriage return
...
def SerReadASCIIResponse(ser, timeout=1):
    ret = ''
    start_time = time.time()
    
    while True:
        # Read available data in chunks (e.g., 64 bytes at once)
        if ser.in_waiting > 0:
            ret += ser.read(ser.in_waiting).decode('utf-8')
        
        # Break on carriage return
        if '\r' in ret:
            ret = ret.split('\r')[0]  # Discard everything after '\r'
            break
        
        # Optional: timeout to avoid infinite loops in case of no response
