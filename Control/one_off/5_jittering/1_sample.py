# -*- coding: utf-8 -*-
"""
Created on Wed Mar 19 10:11:21 2025
...
filepath = 'D:/25_03_10/1_EM04864_01B_80kv_1p2um/'
Subfolder_name="data/"
...
if jitter_flag:
    ...
    if os.path.isfile(filepath + '/jitter.txt'):
        ...
        if lines != num_proj:
            ...
            jitter_sequence = np.round(np.array(jitter_sequence)*px/M,4) # round to 100nm
            jitter_sequence[0] = 0
...
for proj_idx in np.arange(start_proj,num_proj):
    ...
    if jitter_flag:
        if proj_idx == 0:
            AT.AT_move_axis_linear('X',jitter_sequence[proj_idx])
        else:
            AT.AT_move_axis_linear('X',jitter_sequence[proj_idx]-jitter_sequence[proj_idx-1])
